...
export default function EmployeesWidget({ businessId }: { businessId: string }) {
  const [count, setCount] = useState<number | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    let mounted = true
    api.employeesCount(businessId)
  ...
  return (
    <div style={{ background: '#0b1220', padding: '1rem', border: '1px solid #1f2937', borderRadius: 8 }}>
      <div style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Employees</div>
