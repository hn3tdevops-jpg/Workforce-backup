...
from app.models.access_control import (Membership, Permission, Role,
                                       RolePermission, ScopedRoleAssignment)
from app.models.tenant import Business, Location, Tenant
from app.models.user import User
from app.services.rbac_service import (get_active_memberships_for_user,
...
def seed_membership(
    session: Session, user: User, business: Business, *, is_owner: bool = False
) -> Membership:
...
def seed_permission(session: Session, code: str) -> Permission:
    ...
    permission = Permission(
        code=code, resource=resource, action=action, description=code
    )
...
def test_business_wide_role_applies_at_business_and_location_scope() -> None:
    ...
    assert "Manager" in get_effective_role_names(
        session, user.id, business.id, None
    )
    assert "Manager" in get_effective_role_names(
        session, user.id, business.id, location_a.id
    )

    assert (
        user_has_permission(
            session, user.id, "schedule.manage", business.id, None
        )
        is True
    )
    assert (
        user_has_permission(
            session, user.id, "schedule.manage", business.id, location_a.id
        )
        is True
    )
    assert (
        user_has_permission(
            session, user.id, "schedule.manage", business.id, location_b.id
        )
        is True
    )
...
def test_location_scoped_role_only_applies_at_that_location() -> None:
    ...
    assert (
        user_has_permission(
            session, user.id, "hk.tasks.manage", business.id, None
        )
        is False
    )
    assert (
        user_has_permission(
            session, user.id, "hk.tasks.manage", business.id, location_a.id
        )
        is True
    )
    assert (
        user_has_permission(
            session, user.id, "hk.tasks.manage", business.id, location_b.id
        )
        is False
    )


def test_effective_permissions_are_union_of_business_and_location_assignments() -> (
    None
):
    session = make_session()
    _, business, location_a, location_b, user = seed_base(session)
    ...
    business_scope = get_effective_permission_codes(
        session, user.id, business.id, None
    )
    location_a_scope = get_effective_permission_codes(
        session, user.id, business.id, location_a.id
    )
    location_b_scope = get_effective_permission_codes(
        session, user.id, business.id, location_b.id
    )
