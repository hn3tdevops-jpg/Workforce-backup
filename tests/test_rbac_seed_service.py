...
def test_seed_default_roles_creates_expected_roles() -> None:
    ...
    assert {role.name for role in db_roles} == set(DEFAULT_ROLE_PERMISSION_CODES.keys())
...
def test_seed_default_roles_is_idempotent() -> None:
    ...
    role_count = session.scalar(
        select(func.count()).select_from(Role).where(Role.business_id == business.id)
    )
    ...
    assert permission_count == len(set().union(*DEFAULT_ROLE_PERMISSION_CODES.values()))
    assert role_permission_count == sum(len(v) for v in DEFAULT_ROLE_PERMISSION_CODES.values())