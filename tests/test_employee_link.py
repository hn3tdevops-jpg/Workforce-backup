...
from apps.api.app.models.access_control import Permission, Role, RolePermission, Membership, ScopedRoleAssignment
...
async def test_link_user_to_employee_happy_path(client, db_session):
    ...
    await db_session.run_sync(lambda s: s.add(Membership(user_id=actor.id, business_id=business.id, status="active")))
    await db_session.run_sync(lambda s: s.add(RolePermission(role_id=role.id, permission_id=perm.id)))
    # find membership id
    mem = await db_session.run_sync(lambda s: s.scalar(s.query(Membership).filter_by(user_id=actor.id)))
    await db_session.run_sync(lambda s: s.add(ScopedRoleAssignment(membership_id=mem.id, role_id=role.id)))
    await db_session.commit()
    ...
    emp = EmployeeProfile(business_id=business.id, first_name="Jane", last_name="Doe")
    await db_session.run_sync(lambda s: s.add(emp))
    await db_session.commit()

    token = create_access_token(user_id=str(actor.id), business_id=str(business.id))

    resp = await client.post(f"/api/v1/employees/link-user/{target.id}", json={"employee_id": str(emp.id)}, headers={"Authorization": f"Bearer {token}"})
...
async def test_duplicate_link_rejected(client, db_session):
    ...
    emp = EmployeeProfile(business_id=business.id, first_name="John", last_name="Smith")
    await db_session.run_sync(lambda s: s.add_all([actor, target, emp]))
    await db_session.commit()

    # create existing link
    link = UserEmployeeLink(user_id=target.id, employee_id=emp.id, business_id=business.id)
...
async def test_cross_business_link_rejected(client, db_session):
    ...
    token = create_access_token(user_id=str(actor.id), business_id=str(biz_a.id))
